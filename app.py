...
def generate_sensor_string():
    return (
        ...
        f"❤️ Heart Rate: {sensor_data['HEART']} BPM\n"
        f"🌬️ Air Quality (AQI): {sensor_data['AIRQUALITY']}"
    )
...
def health_analysis(issue):
    try:
        ...
        print("⚠️ Health module error:", e)
        return "⚠️ Error generating health advice. Try again later."
...
def handle_message(msg):
    try:
        ...
        if "body temperature" in text:
            ...
            reply_text = f"❤️ Heart Rate: {sensor_data['HEART']} BPM"
        elif "air quality" in text:
            reply_text = f"🌬️ Air Quality (AQI): {sensor_data['AIRQUALITY']}"
        else:
            reply_text = "❓ Ask about body temperature, temperature, humidity, heart rate, air quality, or health."

        speak(reply_text)
    except Exception as e:
        print("⚠️ Telegram handling error:", e)
